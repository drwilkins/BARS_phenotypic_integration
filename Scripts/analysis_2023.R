...
#Run both in a model with sex as a factor
#Belly Chroma
lm_res_both_B<- pbapply::pblapply(1:max(boot_both$boot_i), function(i) {
  pops_boot_i <- boot_both %>% filter(boot_i == i)
...
#Significant effect of chroma
quantile(lm_res_both_B$est_avg_b.chrom,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(lm_res_both_B$est_avg_b.chrom,na.rm=T)

#Nonsignificant effect of sex
quantile(lm_res_both_B$est_sex,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(lm_res_both_B$est_sex,na.rm=T)

# Look at effect of vent on PINT -----------------------------
#Run both in a model with sex as a factor
#Vent Chroma
lm_res_both_V<- pbapply::pblapply(1:max(boot_both$boot_i), function(i) {
  pops_boot_i <- boot_both %>% filter(boot_i == i)
...
#Significant effect of chroma
quantile(lm_res_both_V$est_avg_v.chrom,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(lm_res_both_V$est_avg_v.chrom,na.rm=T)
...
#Look at 95% CIs for females for main result, controlling for fst from egyptian savignii
quantile(
...
both_results_r.chrom <- pbapply::pblapply(1:max(d_gen$boot_i), function(i) {
...
quantile(both_results_r.chrom$est_avg_r.chrom,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_r.chrom$est_avg_r.chrom,na.rm=T,names=F)
#Significant effect of population genetics (Fst) on phenotypic integration (PCIT)
quantile(both_results_r.chrom$est_Fst,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_r.chrom$est_Fst,na.rm=T,names=F)
#Nonsignificant effect of sex on phenotypic integration (PCIT)
quantile(both_results_r.chrom$est_sexM,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_r.chrom$est_sexM,na.rm=T)
...
both_results_t.chrom <- pbapply::pblapply(1:max(d_gen$boot_i), function(i) {
  pops_boot_i <- d_gen %>% filter(boot_i == i)
...
#95CIs for analysis with sex as covariate, rather than splitting up analyses
#Significant effect of breast darkness on phenotypic integration (PCIT)
quantile(both_results_t.chrom$est_avg_t.chrom,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_t.chrom$est_avg_t.chrom,na.rm=T,names=F)
#Significant effect of population genetics (Fst) on phenotypic integration (PCIT)
quantile(both_results_t.chrom$est_Fst,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_t.chrom$est_Fst,na.rm=T,names=F)
#Nonsignificant effect of sex on phenotypic integration (PCIT)
quantile(both_results_t.chrom$est_sexM,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_t.chrom$est_sexM,na.rm=T)

#Running model with sex as a covariate and belly chroma
both_results_b.chrom <- pbapply::pblapply(1:max(d_gen$boot_i), function(i) {
  pops_boot_i <- d_gen %>% filter(boot_i == i)
...
#95CIs for analysis with sex as covariate, rather than splitting up analyses
#Significant effect of breast darkness on phenotypic integration (PCIT)
quantile(both_results_b.chrom$est_avg_b.chrom,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_b.chrom$est_avg_b.chrom,na.rm=T,names=F)
#Significant effect of population genetics (Fst) on phenotypic integration (PCIT)
quantile(both_results_b.chrom$est_Fst,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_b.chrom$est_Fst,na.rm=T,names=F)
#Nonsignificant effect of sex on phenotypic integration (PCIT)
quantile(both_results_b.chrom$est_sexM,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_b.chrom$est_sexM,na.rm=T)

#Running model with sex as a covariate and vent chroma
both_results_v.chrom <- pbapply::pblapply(1:max(d_gen$boot_i), function(i) {
  pops_boot_i <- d_gen %>% filter(boot_i == i)
...
#95CIs for analysis with sex as covariate, rather than splitting up analyses
#Significant effect of breast darkness on phenotypic integration (PCIT)
quantile(both_results_v.chrom$est_avg_v.chrom,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_v.chrom$est_avg_v.chrom,na.rm=T,names=F)
#Significant effect of population genetics (Fst) on phenotypic integration (PCIT)
quantile(both_results_v.chrom$est_Fst,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_v.chrom$est_Fst,na.rm=T,names=F)
#Nonsignificant effect of sex on phenotypic integration (PCIT)
quantile(both_results_v.chrom$est_sexM,probs=c(.025,.975),na.rm=T,names=F,type=7)
mean(both_results_v.chrom$est_sexM,na.rm=T)
