...
selection<-function(dataset,rawtrait,rawfitmetric){ 
  require(ggplot2)
  #remove incomplete rows
  dataset2<-subset(eval(parse(text=dataset)),complete.cases(eval(parse(text=paste(dataset,"$",rawtrait,sep=""))),eval(parse(text=paste(dataset,"$",rawfitmetric,sep="")))))
  trait.Z<-scale(eval(parse(text=paste("dataset2","$",rawtrait,sep=""))))
  fit<-eval(parse(text=paste("dataset2","$",rawfitmetric,sep="")))
  relfit<-fit/mean(fit,na.rm=T)
  newdata<-data.frame(X=trait.Z,Y=relfit)
  model<-lm(relfit~trait.Z+I(trait.Z^2)) #Is this right? Should differential include squared term?
  summ<-summary(model)
#   fitplot<-ggplot(newdata,aes(x=X,y=Y))+theme_bw()+ggtitle(paste("Fitness Curve for",rawtrait,"in",dataset))+
#     theme(plot.title=element_text(face="bold",size=18),axis.text=element_text(size=18),axis.title=element_text(size=18))+xlab(paste("Scaled & Centered",rawtrait))+
#       ylab(paste("Centered",rawfitmetric))+ geom_smooth(method="loess")+geom_point()
    fitplot<-ggplot(newdata,aes(x=X,y=Y))+theme_bw()+ggtitle(dataset)+
    theme(plot.title=element_text(face="bold",size=18),axis.text=element_text(size=18),axis.title=element_text(size=18))+xlab(paste(rawtrait))+
      ylab(paste("w"))+ geom_smooth(method="loess",col="black")+geom_point()
  ...
  stats<-data.frame(Country=dataset,Trait=rawtrait,FitMetric=rawfitmetric,s,se,t.val,p.val,g,se2,t.val2,p.val2)
...
for (i in 1: length(Clist_pcitM)){
  mat<-Clist_pcitM[[i]]
  Q(mat,color="skyblue",border.color="gray20",labels=toi3,shape=shps,vsize=vertsize,vsize2=vertsize,edge.color="black",bg="transparent",label.color="black",label.cex=1.5)
...
for (i in 1: length(Clist_pcitF)){
  mat<-Clist_pcitF[[i]]
  Q(mat,color="salmon",border.color="gray20",labels=toi3,shape=shps,vsize=vertsize,vsize2=vertsize,edge.color="black",bg="transparent",label.color="black",label.cex=1.5)
  mtext(popnames[i],3,line=.5,at=-1.,,adj=.5,col="black")
...
tmp1<-integM
names(tmp1)<-c("darkness","netdensity","clusterdensity","Pop")
tmp1$Sex="M"

tmp2<-integF
names(tmp2)<-c("darkness","netdensity","clusterdensity","Pop")
tmp2$Sex="F"
...
for (i in 1: length(Clist_pcitM)){
  ...
  Q(mat,color="transparent",border.color="transparent",labels=toi3,shape=shps,posCol="transparent",negCol="transparent",vsize=vertsize,lab.col="#181923",plot.new=F,lab.font=2,edge.color="transparent",lab.scale=F,label.cex=.7,label.scale.equal=T,lay="spring")
