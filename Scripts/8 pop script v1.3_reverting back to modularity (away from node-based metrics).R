...
levels(df.comb$Pop)[c(1:4,6)]<-c("CR","IL","RM","TR","NY")
tapply(df.comb$ID,df.comb$Pop,length)#new, better sample sizes

#reorder factor levels west to east
df.comb$Pop<-factor(df.comb$Pop,levels=c("CO","NY","UK","CR","RM","TR","IL","TW"))
...
pcaList<-lapply(1:8,function(x) principal(Dlist[[x]][,toi],rot="varimax",nfactors=5))
...
IIdf<-data.frame(II,pop.netdensity,pop.clusterdensity,pop.nonclusterdensity,meanbri,Pop=names(II))
...
ggplot(data=IIdf,aes(x=meanbri,y=pop.clusterdensity,label=Pop))+geom_point(size=2,aes(col=meanbri))+geom_text(size=5,col="white",x=jitterx,y=jittery)+xlab("Body Brightness Index")+ylab("Integration Index")+stat_ellipse(col="white")+PPTtheme+ylim(c(0,.7))+scale_colour_gradient(limits=c(.3, .50), low="#FFFFCC", high="#CC6600",guide=F)+annotate("text",x=.32,y=.6,adj=0,label="rho= .809, p= 0.022",col="white",size=5) #paste0("rho == ~-.833~ p== 0.015"),parse=T,col="white")
