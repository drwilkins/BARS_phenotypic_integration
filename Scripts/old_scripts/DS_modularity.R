...
mods.list.male=lapply(corr_list_males, function(x) {
  ...
  btw_mod1=mean(abs(x[which(patches==1), which(patches!=1)]))
  btw_mod2=mean(abs(x[which(patches==2), which(patches!=2)]))
  btw_mod3=mean(abs(x[which(patches==3), which(patches!=3)]))
...
mod.dat=integ %>% select(starts_with("wi"), starts_with("btw"), ends_with("chrom"), sex) %>% 
  rename(wi_t=wi_mod1, wi_r=wi_mod2, wi_b=wi_mod3, wi_v=wi_mod4, bt_t=btw_mod1, bt_r=btw_mod2, bt_b=btw_mod3, bt_v=btw_mod4) %>%
  ...
  mutate(patch = replace(patch, patch=="r", "breast")) %>%
  mutate(patch = replace(patch, patch=="b", "belly")) %>%
  mutate(patch = replace(patch, patch=="v", "vent"))
