...
Clist_pcit_bin<-lapply(1:8,function(x) apply(Clist_pcit2[[x]],1, function(xx) ifelse(xx==0,0,1)))#binary correlation matrices (as list)
...
diag(meanNet)<-NA
length(Q(meanNet[reorganize,reorganize])$Edgelist[[1]])#79 edges
...
icol<-blacks[k2]
    ...
    Q(mat,title=names(Clist_pcit2)[i],posCol=1,labels=toi2[reorganize],shape=shps[reorganize],vsize=10,vsize2=10,edge.color=icol)
  PCarcs(pcaList[[i]],pcaList[[4]])
  mtext(paste0("n=",pcaList[[i]]$n.obs),side=1,line=-.75,at=1.1,cex=.9,font=1)
}
dev.off()
...
for(i in 1:8){
  ...
  trans[i]<-transitivity(sub,"undirected")
}
...
par(mar=c(3,6,3,3))
#circular layouts
pdf("figs/8 pop modularity w diff algorithms_circles (larger sample).pdf",onefile=T,height=8,width=10,colormodel="cmyk")
plotcomm(iCO.comms,lay,"Colorado")
...
pdf("figs/8 pop modularity w diff algorithms_F-R lay (larger sample).pdf",onefile=T,height=8,width=10,colormodel="cmyk")
...
Z<-cbind(machos[,c("ID","Sex","Pop","Year")],apply(machos[,toi],2,scale))#scales traits of interest across all pops
(Zmeans<-aggregate(.~Pop,data=Z[,c("Pop",toi)],mean)) #Z-scaled means for each trait in each pop

#generate all pairwise combos for each country
comparison<-combn(8,2) #used for comparing differences in means and degrees
Z.meandiff<-apply(Zmeans[,-1],2,function(trait) apply(comparison,2,function(dyad) trait[dyad][1]-trait[dyad][2] ))
Z.meandiff<-data.frame(Dyads=apply(comparison, 2, function(x) paste0(Zmeans$Pop[x[1]],"-",Zmeans$Pop[x[2]])),Z.meandiff)
Z.meandiff

###########
## OK, now get degree diffs
